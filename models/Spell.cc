...
const std::string Spell::tableName = "spell";

const std::vector<typename Spell::MetaData> Spell::metaData_={
...
Spell::Spell(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        if(!r["Id"].isNull())
...
Spell::Spell(const Json::Value &pJson) noexcept(false)
{
...
void Spell::updateByJson(const Json::Value &pJson) noexcept(false)
{
    if(pJson.isMember("Id"))
    {
        if(!pJson["Id"].isNull())
...
const int32_t &Spell::getValueOfId() const noexcept
{
...
const std::shared_ptr<int32_t> &Spell::getId() const noexcept
{
    return id_;
}
void Spell::setId(const int32_t &pId) noexcept
{
    id_ = std::make_shared<int32_t>(pId);
    dirtyFlag_[0] = true;
}
const typename Spell::PrimaryKeyType & Spell::getPrimaryKey() const
{
    assert(id_);
    return *id_;
}

const std::string &Spell::getValueOfName() const noexcept
{
...
const std::shared_ptr<std::string> &Spell::getName() const noexcept
{
    return name_;
}
void Spell::setName(const std::string &pName) noexcept
{
    name_ = std::make_shared<std::string>(pName);
    dirtyFlag_[1] = true;
}
void Spell::setName(std::string &&pName) noexcept
{
    name_ = std::make_shared<std::string>(std::move(pName));
    dirtyFlag_[1] = true;
}

const std::string &Spell::getValueOfDescription() const noexcept
{
    const static std::string defaultValue = std::string();
    if(description_)
        return *description_;
...
const std::shared_ptr<std::string> &Spell::getDescription() const noexcept
{
    return description_;
}
void Spell::setDescription(const std::string &pDescription) noexcept
{
    description_ = std::make_shared<std::string>(pDescription);
    dirtyFlag_[2] = true;
}
void Spell::setDescription(std::string &&pDescription) noexcept
{
    description_ = std::make_shared<std::string>(std::move(pDescription));
    dirtyFlag_[2] = true;
}
void Spell::setDescriptionToNull() noexcept
{
    description_.reset();
    dirtyFlag_[2] = true;
}
...
const std::shared_ptr<int32_t> &Spell::getPlayerid() const noexcept
{
    return playerid_;
}
...
const std::vector<std::string> &Spell::insertColumns() noexcept
{
...
void Spell::outputArgs(drogon::orm::internal::SqlBinder &binder) const
{
    if(dirtyFlag_[1])
    {
        if(getName())
...
const std::vector<std::string> Spell::updateColumns() const
{
...
void Spell::updateArgs(drogon::orm::internal::SqlBinder &binder) const
{
    if(dirtyFlag_[1])
    {
        if(getName())
...
Json::Value Spell::toMasqueradedJson(
...
bool Spell::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    if(pJson.isMember("Id"))
    {
    ...
    {
        err="The PlayerId column cannot be null";
        return false;
    }
    return true;
}
bool Spell::validateMasqueradedJsonForCreation(const Json::Value &pJson,
...
bool Spell::validateJsonForUpdate(const Json::Value &pJson, std::string &err)
{
    if(pJson.isMember("Id"))
    {
...
bool Spell::validateMasqueradedJsonForUpdate(const Json::Value &pJson,
                                             const std::vector<std::string> &pMasqueradingVector,
                                             std::string &err)
{
    if(pMasqueradingVector.size() != 4)
...
bool Spell::validJsonOfField(size_t index,
